/*
  ...
  k-means implementation with equal cluster size option. It takes a an array of objects with x/y properties, assignes
  each object to a cluster and adds a property k.
  
  borrowed from 
    - cmtt: https://github.com/cmtt/kmeans-js
    - ELKI: http://elki.dbs.ifi.lmu.de/browser/elki/trunk/src/tutorial/clustering/SameSizeKMeansAlgorithm.java

  TODO
    - settings: maxiter
    - optimize: remove assoziative items array, lots of room for speed improvements ... too lazy while porting java code
...
/*
    if (equalSize) {
      ...
      for (var k = 0; k < ks; k++) {
        var centroid = centroids[k];
        var items = centroid.data;
        var dif_x = 0, dif_y = 0;
        for (var id in items) {
          if (items.hasOwnProperty(id)) {
            var item = items[id];
            dif_x += pow(item.x - centroid.x, 2);
            dif_y += pow(item.y - centroid.y, 2);
          }
        }
        sum += dif_x + dif_y;
      }
